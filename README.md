...
- **Unobserved confounders invalidate the result.** DML identifies a causal effect only if all confounders — variables that drive both treatment and outcome — are included. In insurance, key unobservables include actual annual mileage, telematics behaviour not captured in the score, and claim reporting propensity. These are not in the data. Use `sensitivity_analysis()` to understand how robust the conclusion is; if the result only holds at gamma=1.0 (no unobserved confounding), treat it as exploratory.

- **Near-deterministic treatment leaves nothing to identify from.** When price changes are generated by a deterministic rule (e.g. a pure technical rate engine with no manual overrides), D is very close to a function of X. The residualised treatment D_tilde has near-zero variance. The DML estimate becomes imprecise and the confidence interval wide — correctly reflecting that the observational data contain no usable exogenous variation. The only remedy is to exploit genuine sources of randomness: manual underwriting decisions, timing effects, competitive shocks, or an actual A/B test. There is no statistical fix for an identification problem.

- **Mediators in the confounder list attenuate the estimate.** Including a variable causally downstream of the treatment (a mediator) as a confounder blocks the causal channel you are trying to measure — the "bad controls" problem. NCD is a common example: it is partly caused by prior claims history, which is partly caused by the risk factors you are studying. Including NCD as a confounder will absorb some of the treatment effect you are trying to estimate. Think carefully about the causal graph before adding variables to the confounders list. A directed acyclic graph (DAG) is the right tool for this exercise.

- **Minimum effective sample size is approximately 2,000.** Below n=2,000 the sample-size-adaptive CatBoost schedule still runs, but confidence intervals are too wide to be commercially useful — typically ±50–100% of the point estimate. At n < 500 per segment in `cate_by_segment()`, estimates are unreliable regardless of parameter tuning. If the analysis segment is genuinely this small, consider pooling via a hierarchical model (see `bayesian-pricing`) rather than DML.

- **The partially linear regression model assumes a homogeneous treatment effect.** The `CausalPricingModel` PLR estimator produces a single theta_0, which is the average causal effect. If the true treatment effect is genuinely heterogeneous — elasticity for NCD=0 customers is three times that for NCD=5 — theta_0 is a population average that may not represent any actual subgroup. Use the `causal_forest` subpackage when heterogeneous effects are the object of interest.

- **The sensitivity analysis is a heuristic, not a Rosenbaum bound.** The `sensitivity_analysis()` function uses `bias_bound = log(gamma) * se`, which is an approximation applied to the DML standard error. It is not the classical Rosenbaum rank-based test. For results that will be cited in regulatory submissions or academic work, use the `sensemakr` package (Cinelli-Hazlett 2020 partial R-squared bounds) in addition.

- **Causal clustering is an approximation above n=10,000.** `CausalClusteringAnalyzer` computes the forest proximity kernel on a 5,000-observation subsample for larger datasets and assigns the remainder by nearest-neighbour. The cluster boundaries are approximate and may shift by 5–10% of observations relative to the exact solution. For definitive segmentation decisions, run on the full dataset by reducing the sample size fed to the forest, or accept that cluster composition is approximate.

- **This library does not handle panel data, instrumental variables, or regression discontinuity.** If your identification strategy requires exploiting a regulatory threshold (RD), a price experiment with exclusion restriction (IV), or repeated observations of the same policyholder over time (difference-in-differences), the DML framework implemented here is not the right tool. Use `insurance-causal-policy` for SDID-based policy evaluation, or a dedicated IV library for exclusion-restriction designs.
